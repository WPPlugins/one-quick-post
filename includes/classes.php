...
class Oqp_Form {
	...
	function __construct($page_id=false,$options=false) {
		global $oqp_form;
		...
		$page_id=intval($page_id);
                ...
                if ($page_id){
			$db_options = self::get_options($page_id);
                        if(!$db_options) unset($page_id);
...
class Oqp_Post extends Oqp_Form {
        ...
        var $requested_step;
        var $allowed_post_statuses=array('publish','pending','draft');
	...
	function __construct($post_id,$page_id=false) {
		...
		if ($this->requested_post_id){
                ...
                //allow users to view single posts even if they have not post_status="publish"
                add_filter('oqp_pre_get_posts',array(&$this,'show_unpublished_post'),1);

                //reload hidden posts
                add_filter('the_posts',array(&$this,'reload_unpublished_post'),1);
        ...
        function show_unpublished_post($query){
            
            if(!is_single()) return $query;
            if (!$query->is_main_query())return $query;

            if(!is_oqp_post($loaded_post->ID)) return $query;

            $query->set( 'post_status',$this->allowed_post_statuses);

            oqp_debug($this->allowed_post_statuses,"allowed post statuses");

            return $query;

        }

        function reload_unpublished_post($posts){
            global $wp_query, $wpdb;

            //if(!oqp_is_single()) return $query;
            if(!oqp_is_single()) return $posts;
            if (!$wp_query->is_main_query())return $posts;      
            //if(!$this->guest['key']) return $posts;
            ...
            oqp_debug($posts[0],"guest_reload_hidden_single_post","warn");
	...
	function form_check_capability($steps){
        ...
        /*
	function check_user_can_edit_post($post_id){
                global $oqp_form;
		//POST CREATION
		$cap_needed = 'edit_posts';
                
                //USER
                if(!$user_id) $user_id=get_current_user_id();
                $user_id=apply_filters('oqp_get_current_user_id',$user_id);
                
                $post = get_post($post_id);


                
                $can_edit = apply_filters("oqp_user_can_edit_post",$can_edit,$post_id,$user_id,$cap_needed);

                oqp_debug(array('user_id'=>$user_id,'post_id'=>$post_id,'can'=>$can_edit),'check_user_can_edit_post');
	...
	function __construct($post_id=false,$page_id=false) {
        ...
        function init(){
               //add "oqp_creation_post_actions" hook
                ...
                add_action('oqp_creation_post_actions',array(&$this,'redirect_reset_form'));

                //no capability to create new posts
                add_action('oqp_creation_post_actions',array(&$this,'redirect_cannot_create'));

                //no capability to edit current post
                add_action('oqp_creation_post_actions',array(&$this,'redirect_cannot_edit'));

                //cannot "admin" draft post (redirect to "create")
                add_action('oqp_creation_post_actions',array(&$this,'redirect_cannot_admin_draft'));

                //cannot "create" non-draft post (redirect to "create")
                add_action('oqp_creation_post_actions',array(&$this,'redirect_cannot_create_non_draft'));
		...
		if ($post_saved && $no_errors) { //all went well, tell them !
			...
			if (did_action( 'oqp_validated_last_step' )) { //post creation finished
				...
				if ($no_messages){
                                ...
                                if($this->action==_x('create','slug','oqp')){
                                    $url_args['step_key']=$this->current_step+1;
                                }
			}
