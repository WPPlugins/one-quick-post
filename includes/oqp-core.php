...
/*
retrieves the YCLAD args set in a query
*/
/*
function oqp_filter_query_args($query=false){
	global $wp_query;
	...
	//put all query args in an array
	
	$args = $query->query_vars;
	$args = yclads_filter_args($args);	
	
	return apply_filters('oqp_filter_query_args',$args);
...
function oqp_pre_get_posts_get_singular_id(){
    //TO FIX
...
function oqp_detect_page_type($query){
    ...
    oqp_debug($p,"get_queried_object_id","warn");
    
    if(is_oqp_post($p)){
        oqp_debug($p,"IS OQP POST","warn");
        ...
        if(Oqp_Form::get_options($p)){
            
            if(in_array($action,$valid_actions)){
                ////CREATION FORM - EMPTY POST
                oqp_debug($p,"IS OQP CREATION PAGE","warn");

                $oqp_form = new Oqp_Creation_Page(false,$p);

            }else{
                ////DIRECTORY
                oqp_debug($p,"IS OQP DIRECTORY","warn");
                $oqp_form = new Oqp_Form($p);
